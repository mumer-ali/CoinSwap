...
export default function Form() {
    ...
    const fetchKeys = async () => {
      try {
        const response = await fetch(
          "https://v6.exchangerate-api.com/v6/b50e62b8b8bfbc73fada46a0/latest/PKR"
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `https://v6.exchangerate-api.com/v6/b50e62b8b8bfbc73fada46a0/latest/${from}`
      );
      const data = await response.json();
      let exRate = data["conversion_rates"][to] * Number(input);
      setOutput(exRate.toFixed(2));
    };
    if (Number(input) >= 0) {
      fetchData();
    } else {
        ...
        <div className="bg-[rgba(133,122,122,0.5)] flex flex-col h-48 w-64 justify-center items-center rounded-lg border-2">
          <div className="flex justify-center items-center">
            <input
              ...
              name="from"
              placeholder="From:"
              className="outline-none w-38 h-10 p-2 text-sm bg-white"
              value={input}
              onChange={(e) => setInput(e.target.value)}
            />
            <select
              className="outline-none h-10 w-14 text-xs"
              value={from}
